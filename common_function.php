...
function getproducts(){
    ...
    if(!isset($_GET['category'])){
        if(!isset($_GET['brand'])){
            ...
            $select_query = "SELECT * FROM products ORDER BY date DESC LIMIT 32";
  ...
  if (isset($_GET['product_id'])) {
      if (!isset($_GET['category']) && !isset($_GET['brand'])) {
          ...
          while ($row = mysqli_fetch_assoc($result_query)) {
              $product_title = $row['product_title'];
              $product_description = $row['product_description'];
              $product_image1 = $row['product_image1'] ?? 'default.jpg';
              $product_image2 = $row['product_image2'] ?? 'default.jpg';
              $product_image3 = $row['product_image3'] ?? 'default.jpg';
              $product_email = $row['product_email'];
              $product_contact = $row['product_contact'];
              $product_price = $row['product_price'];
              $district = $row['district'];

              // Store all image URLs in an array for JavaScript navigation
              $images = json_encode([$product_image1, $product_image2, $product_image3]);

              echo "
              <div class='col-md-4 mb-2'>
                  <div class='card'>
                      <img src='./admin_area/product_images/$product_image1' class='card-img-top' alt='$product_title' onclick='showModal(0)'>
                      <div class='card-body'>
              ...
              <div class='col-md-8'>
                  <div class='row'>
                      ...
                      <div class='col-md-6'>
                          <img onclick='showModal(1)' width='100' src='./admin_area/product_images/$product_image2' class='card-img-top' alt='$product_title'>
                      </div>
                      <div class='col-md-6'>
                          <img onclick='showModal(2)' width='100' src='./admin_area/product_images/$product_image3' class='card-img-top' alt='$product_title'>
                      </div>
                  </div>
              </div>
              ...
              <script>
                  var images = $images; // Load images array from PHP
                  var currentIndex = 0;
                  ...
                  function showModal(index) {
                      ...
                      modalImg.src = './admin_area/product_images/' + images[currentIndex];
                      caption.innerHTML = images[currentIndex];
                  }
