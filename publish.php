...
if (isset($_POST['insert_product'])) {
    ...
    $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'jfif'];

    // Check for valid image extensions
    function is_valid_image($image_name) {
        global $allowed_extensions;
        $image_extension = strtolower(pathinfo($image_name, PATHINFO_EXTENSION));
        return in_array($image_extension, $allowed_extensions);
    }

    // Check if all required fields are filled
    if (empty($product_title) || empty($description) || empty($product_keywords) || empty($product_category) ||
        empty($product_brands) || empty($product_price) || empty($district) || empty($product_email) ||
        empty($product_contact) || empty($product_image1) || empty($product_image2) || empty($product_image3)) {
        echo "<script>alert('Please fill all required fields')</script>";
        exit();
    } elseif (!is_valid_image($product_image1) || !is_valid_image($product_image2) || !is_valid_image($product_image3)) {
        echo "<script>alert('Only JPG, JPEG, and PNG files are allowed for images')</script>";
        exit();
    } else {
        // Move uploaded files
